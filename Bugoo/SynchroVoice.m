...
@interface SynchroVoice()<CBCentralManagerDelegate, CBPeripheralDelegate>
{
    NSTimer *timer;
    UIView *syView;
    UIButton *sybutton;
...
@implementation SynchroVoice

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = HXString(@"语音棒同步");
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:HXString(@"操作提示") style:UIBarButtonItemStyleDone target:self action:@selector(help)];
...
-(void)createUI{
    ...
    _timeLabel = [[UILabel alloc]initWithFrame:CGRectMakeIPHONE6Adapt(0, 453, 375, 20)];
    _timeLabel.textColor = [Utility colorWithHexString:@"007acc"];
    _timeLabel.font = [UIFont systemFontOfSize:14];
    _timeLabel.textAlignment = NSTextAlignmentCenter;
    if (Time&&self.model.isVoice) {
...
-(void)help{
    UIAlertController *help = [UIAlertController alertControllerWithTitle:HXString(@"") message:HXString(@"1、将语音棒插入有效的USB电源\n2、长按与语音棒的灰色按钮，听到\"哔\"声后进入蓝牙配对模式\n3、出现设备后，点击\"开始同步\"") preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *search = [UIAlertAction actionWithTitle:HXString(@"开始搜索") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self scan:nil];
    } ];
...
- (void)scan:(UIButton *)button{
    ...
    _searchView.string =HXString(@"搜索设备中");
...
//        [sybutton setTitleColor:[Utility colorWithHexString:@"55555"] forState:UIControlStateNormal];
//        sybutton.backgroundColor = [Utility colorWithHexString:@"d8d8d8"];
...
-(void)time{
    ...
    [Utility topAlertView:HXString(@"搜索失败")];
}
...
-(void)click:(UIButton *)button{
    if ([button.currentTitle isEqualToString:HXString(@"开始同步")]) {
        ...
        detail.text = HXString(@"1、语音棒需接入电源 \n2、请勿强制退出APP \n3、确保语音棒为蓝牙配对模式");
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"FD01"]]) {
        ...
        if ([read isEqualToString:@"74"]) {
            ...
            _timeLabel.text = [NSString stringWithFormat:@"%@(%@)",HXString(@"最近同步时间"),[Utility getTime:date]];
            DEFAULTS_INFO(_nickName.text, @"VoiceName");
            DEFAULTS_INFO(_timeLabel.text, @"time");
...
//        else if([read isEqualToString:@"C4"]){
//            [self write:3];
//        }else if([read isEqualToString:@"A4"]){
//           
//            
//        }
    }
...
//    else if (tireID==2){
//        
//        string_content = [NSString stringWithFormat:@"5560000%@00%@00%@%@%@",_model.rfid,RH,RL,temp,unitString];
//        
//    }else if(tireID==3){
//        
//        string_content = [NSString stringWithFormat:@"5540000%@00%@00%@%@%@",@"FFFFF",RH,RL,temp,unitString];
//        
//    }
    ...
    init_crc8_table1();
    crc8Init1(&crc8Result);
    crc8Update1(&crc8Result, a, 12);
    crc8Finish1(&crc8Result);
...
unsigned char crc8table1[256];
void init_crc8_table1() {
    //#if endian8
...
void crc8Init1(unsigned char *pCrc8) {
    *pCrc8 = Crc8_Init;
}
void crc8Update1(unsigned char *pCrc8, unsigned char *pData, unsigned int uSize){
    //#if endian8
    for (unsigned int i = 0; i < uSize; i++)
        *pCrc8 = crc8table1[pData[i] ^ *pCrc8];
...
void crc8Update_Direct1(unsigned char *pCrc8, unsigned char *pData, unsigned int uSize) {
    //#if endian8
    //code needed
    //#else
...
void crc8Finish1(unsigned char *pCrc8) {
    *pCrc8 ^= Crc8_XorOut;
}
